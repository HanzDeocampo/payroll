...
<head>
    ...
    <title><?php echo isset($pageTitle) ? $pageTitle . ' - ' : ''; ?>Payroll System | City Of Bacolod</title>
...
<div class="app-container">
    ...
    <aside class="sidebar" id="appSidebar">
        ...
        <div class="sidebar-header">
            ...
            <p class="sidebar-subtitle">City of Bacolod</p>
        </div>
...
(function(){
    ...
    if(el){
        ...
        if(saved){el.style.scrollBehavior='auto';el.scrollTop=parseInt(saved,10);el.style.scrollBehavior='';}
        el.addEventListener('scroll',function(){sessionStorage.setItem(key,el.scrollTop);});
    }
    // ── Sidebar-nav scroll ──
    var NAV_KEY='sidebar_nav_scroll';
    ...
    function restoreNav(){
        ...
        if(s){nav.style.scrollBehavior='auto';nav.scrollTop=parseInt(s,10);nav.style.scrollBehavior='';}
    }
    function saveNav(){var nav=getNav();if(nav)sessionStorage.setItem(NAV_KEY,nav.scrollTop);}
    document.addEventListener('DOMContentLoaded',restoreNav);
    window.addEventListener('load',restoreNav);
    window.addEventListener('beforeunload',saveNav);
    document.addEventListener('click',function(e){
        if(e.target.closest('#appSidebar a:not(.sidebar-logout)'))saveNav();
    });
})();
</script>