...
<form method="POST" id="batchForm">
    ...
    <!-- Restore notice -->
    <div id="restore-notice" style="display:none;background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:.65rem 1rem;margin-bottom:1rem;font-size:.82rem;color:#1d4ed8;align-items:center;justify-content:space-between;gap:1rem;">
        <span><i class="fas fa-history"></i> &nbsp;Previous session data restored. Your loan entries and period selection have been reloaded.</span>
        <button type="button" onclick="clearFormState(true)" style="background:#dbeafe;border:1px solid #93c5fd;color:#1d4ed8;border-radius:6px;padding:.3rem .8rem;font-size:.78rem;cursor:pointer;white-space:nowrap;"><i class="fas fa-trash-alt"></i> Clear saved data</button>
    </div>
        ...
        <div class="dept-loan-totals-body" id="deptLoanTotalsBody">
            ...
            <div class="dept-emp-breakdown">
                ...
                <table class="dept-emp-table" id="deptEmpBreakdownTable">
                    <thead>
                        <tr>
                            <th>Employee</th><th>GSIS</th><th>Pag-IBIG</th><th>Provident</th>
                            <th>BCGEU</th><th>BACGEM</th><th>NOCGEM</th><th>Other</th><th>Total</th>
                        </tr>
                    </thead>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td id="dft-gsis">—</td><td id="dft-pagibig">—</td><td id="dft-provident">—</td>
                            <td id="dft-bcgeu">—</td><td id="dft-bacgem">—</td><td id="dft-nocgem">—</td>
                            <td id="dft-other">—</td><td id="dft-grand">—</td>
                        </tr>
                    </tfoot>
                </table>
                </div>
...
const PREFIXES  = ['gsis','pagibig','provident','bacgem','nocgem','other'];
var empBaseSalaries = {};
<?php foreach ($employees as $emp): ?>
...
function fmtDT(n) { return '₱' + parseFloat(n).toLocaleString('en-PH',{minimumFractionDigits:2,maximumFractionDigits:2}); }
...
function checkLoanVsSalary(empId){
    var salary=parseFloat((document.getElementById('basic-salary-'+empId)||{}).value)||0;
    var totalLoans=0;
    document.querySelectorAll('.emp-loan-input[data-empid="'+empId+'"]').forEach(function(inp){totalLoans+=parseFloat(inp.value)||0;});
    ...
    if(warn){if(over){var diff=totalLoans-salary;document.getElementById('emp-warn-text-'+empId).textContent='Loans ('+fmt(totalLoans)+') exceed salary ('+fmt(salary)+') by '+fmt(diff);warn.classList.add('visible');}else{warn.classList.remove('visible');}}
...
function refreshGlobalWarning(){
    ...
    if(overEmp.length>0){
        ...
        overEmp.forEach(function(id){
            ...
            var totalLoans=0;
            document.querySelectorAll('.emp-loan-input[data-empid="'+id+'"]').forEach(function(inp){totalLoans+=parseFloat(inp.value)||0;});
        ...
        banner.classList.add('visible');bar.classList.add('has-warnings');
        warnNum.textContent=overEmp.length;warnSpan.style.display='inline';
    }else{
        banner.classList.remove('visible');bar.classList.remove('has-warnings');
...
function updateEmpCoopTotal(empId,prefix){
    ...
    let grand=0;
    PREFIXES.forEach(p=>{document.querySelectorAll('.emp-'+empId+'-'+p+'-sub').forEach(i=>{grand+=parseFloat(i.value)||0;});});
...
function getBcgeuForPeriod(){var p=document.getElementById('period_type')?document.getElementById('period_type').value:'1-31';return(p==='1-15'||p.startsWith('16-'))?50.00:100.00;}
...
function refreshDeptTotals(){
    ...
    deptEmpIds.forEach(function(empId){
        ...
        PREFIXES.forEach(function(pfx){
            ...
            var t=0;inputs.forEach(function(inp){t+=parseFloat(inp.value)||0;});empSums[pfx]=t;
        });
        var empTotal=empSums.gsis+empSums.pagibig+empSums.provident+empSums.bcgeu+empSums.bacgem+empSums.nocgem+empSums.other;
        ['gsis','pagibig','provident','bcgeu','bacgem','nocgem','other'].forEach(function(pfx){
            var cell=document.getElementById('dept-row-'+pfx+'-'+empId);
            if(!cell)return;
            var v=empSums[pfx];
            if(v>0){cell.textContent=fmtDT(v);cell.classList.remove('dt-zero-cell');cell.style.color='';}
            else{cell.textContent='—';cell.classList.add('dt-zero-cell');}
        });
        var totCell=document.getElementById('dept-row-total-'+empId);
        if(totCell){if(empTotal>0){totCell.textContent=fmtDT(empTotal);totCell.classList.remove('dt-zero-cell');totCell.style.fontWeight='700';totCell.style.color='#1a3d6b';}else{totCell.textContent='—';totCell.classList.add('dt-zero-cell');totCell.style.color='';}}
        var row=document.getElementById('dept-emp-row-'+empId);
        if(row)row.style.opacity=isSelected?'':'0.35';
        if(isSelected){selectedCount++;Object.keys(deptSums).forEach(function(k){if(k!=='grand')deptSums[k]+=empSums[k];});}
    });
    deptSums.grand=deptSums.gsis+deptSums.pagibig+deptSums.provident+deptSums.bcgeu+deptSums.bacgem+deptSums.nocgem+deptSums.other;
    var cardMap={gsis:'dtt-gsis',pagibig:'dtt-pagibig',provident:'dtt-provident',bcgeu:'dtt-bcgeu',bacgem:'dtt-bacgem',nocgem:'dtt-nocgem',other:'dtt-other'};
    Object.entries(cardMap).forEach(function([k,elId]){var el=document.getElementById(elId);if(el)el.textContent=fmtDT(deptSums[k]);});
    var grandEl=document.getElementById('dtt-grand');if(grandEl)grandEl.textContent=fmtDT(deptSums.grand);
    var cntEl=document.getElementById('dtt-selected-count');if(cntEl)cntEl.textContent=selectedCount+' selected';
    var footMap={gsis:'dft-gsis',pagibig:'dft-pagibig',provident:'dft-provident',bcgeu:'dft-bcgeu',bacgem:'dft-bacgem',nocgem:'dft-nocgem',other:'dft-other'};
    Object.entries(footMap).forEach(function([k,elId]){var el=document.getElementById(elId);if(el){el.textContent=deptSums[k]>0?fmtDT(deptSums[k]):'—';}});
    var gfEl=document.getElementById('dft-grand');if(gfEl)gfEl.textContent=deptSums.grand>0?fmtDT(deptSums.grand):'—';
}
...
document.addEventListener('change',function(e){if(e.target&&e.target.classList.contains('emp-checkbox')){refreshDeptTotals();saveFormState();}});
document.getElementById('period_type')&&document.getElementById('period_type').addEventListener('change',function(){refreshDeptTotals();saveFormState();});
document.getElementById('selectAll')&&document.getElementById('selectAll').addEventListener('change',function(){refreshDeptTotals();saveFormState();});

// ── localStorage persistence ──────────────────────────────────────────────────
function saveFormState(){
    try{
        var state={};
        ['payroll_month','payroll_year','period_type'].forEach(function(id){
            var el=document.getElementById(id);if(el)state[id]=el.value;
        });
        document.querySelectorAll('.emp-checkbox').forEach(function(el){
            state['chk_'+el.value]=el.checked;
        });
        document.querySelectorAll('input[name^="basic_salaries["], input[name^="peras["]').forEach(function(el){
            state[el.name]=el.value;
        });
        document.querySelectorAll('.emp-loan-input').forEach(function(el){
            state[el.name]=el.value;
        });
...
function restoreFormState(){
    try{
        ...
        ['payroll_month','payroll_year'].forEach(function(id){
            var el=document.getElementById(id);
            if(el&&state[id]!==undefined)el.value=state[id];
        });
        updatePeriodOptions(state['period_type']||'1-31');

        document.querySelectorAll('.emp-checkbox').forEach(function(el){
            var saved=state['chk_'+el.value];
        ...
        deptEmpIds.forEach(function(empId){
            PREFIXES.forEach(function(pfx){
                var t=0;
                document.querySelectorAll('.emp-'+empId+'-'+pfx+'-sub').forEach(function(i){t+=parseFloat(i.value)||0;});
                var totalEl=document.getElementById('emp-'+pfx+'-total-'+empId);
                if(totalEl)totalEl.textContent=fmt(t);
                if(t>0){
                    var body=document.getElementById('emp-'+pfx+'-body-'+empId);
                    var chev=document.getElementById('emp-'+pfx+'-chevron-'+empId);
                    if(body)body.classList.add('open');
                    if(chev)chev.classList.add('open');
                }
            });
            var grand=0;
            PREFIXES.forEach(function(p){document.querySelectorAll('.emp-'+empId+'-'+p+'-sub').forEach(function(i){grand+=parseFloat(i.value)||0;});});
            var badge=document.getElementById('emp-loan-badge-'+empId);
            if(badge){
                badge.innerHTML='<i class="fas fa-minus-circle" style="font-size:.6rem"></i> '+fmt(grand);
                badge.classList.toggle('has-value',grand>0);
            }
            var panelWasOpen=localStorage.getItem('emp_panel_open_'+empId)==='1';
            if(panelWasOpen||grand>0){
                var row=document.getElementById('emp-loan-row-'+empId);
                var chev=document.getElementById('emp-chevron-'+empId);
...
function clearFormState(showConfirm){
    if(showConfirm&&!confirm('Clear all saved data and reset the form to defaults?'))return;
    try{
        localStorage.removeItem(STORE_KEY);
        deptEmpIds.forEach(function(empId){localStorage.removeItem('emp_panel_open_'+empId);});
    }catch(e){}
    var notice=document.getElementById('restore-notice');
    if(notice)notice.style.display='none';
    document.querySelectorAll('.emp-loan-input').forEach(function(el){el.value='0.00';});
    deptEmpIds.forEach(function(empId){
        PREFIXES.forEach(function(pfx){
            var totalEl=document.getElementById('emp-'+pfx+'-total-'+empId);
        ...
        var badge=document.getElementById('emp-loan-badge-'+empId);
        if(badge){badge.innerHTML='<i class="fas fa-minus-circle" style="font-size:.6rem"></i> ₱0.00';badge.classList.remove('has-value','over-salary');}
        var warn=document.getElementById('emp-warn-'+empId);
        if(warn)warn.classList.remove('visible');
        loanWarnings[empId]=false;
    });
    refreshDeptTotals();
    refreshGlobalWarning();
    updatePeriodOptions('1-31');
}

document.querySelectorAll('#batchForm input, #batchForm select').forEach(function(el){
    el.addEventListener('input',saveFormState);
    el.addEventListener('change',saveFormState);
});

document.addEventListener('DOMContentLoaded',function(){
    var restored=restoreFormState();
    if(!restored){
        updatePeriodOptions('1-31');
        updatePeraLimits();
    }
...
document.getElementById('batchForm').addEventListener('submit',function(e){
    ...
    if(overCount>0){if(!confirm('⚠️ Warning: '+overCount+' employee(s) have loan deductions exceeding their salary.\n\nThis will result in a negative net pay.\n\nDo you still want to proceed?')){e.preventDefault();return false;}}
...
function printEmpLoans(empId,fullName,employeeId,position,sg,step,basicSalary){
    ...
    var blocks={'GSIS Loans':['gsis_life_ret','gsis_emergency','gsis_cpl','gsis_gpal','gsis_mpl','gsis_mpl_lite','gsis_policy_loan'],'Pag-IBIG Loans':['pagibig_multi','pagibig_emergency','pagibig_premium','pagibig_mp2','pagibig_housing'],'Provident Fund':['provident_fund','provident_fund_loan','provident_edu_loan','provident_term_loan'],'BACGEM Coop':['bacgem_edu_loan','bacgem_grocery','bacgem_others','bacgem_hcp','bacgem_loan'],'NOCGEM Coop':['nocgem_edu_loan','nocgem_emergency','nocgem_grocery','nocgem_hospital','nocgem_others','nocgem_plp','nocgem_regular_loan'],'Misc. Deductions':['other_cash_advance','other_insurance','other_salary_loan','other_others']};
    var labels={'gsis_life_ret':'GSIS Life & Ret.','gsis_emergency':'GSIS Emergency','gsis_cpl':'GSIS CPL','gsis_gpal':'GSIS GPAL','gsis_mpl':'GSIS MPL','gsis_mpl_lite':'GSIS MPL Lite','gsis_policy_loan':'GSIS Policy Loan','pagibig_multi':'Pag-IBIG Multi-Purpose','pagibig_emergency':'Pag-IBIG Emergency','pagibig_premium':'Pag-IBIG Premium','pagibig_mp2':'Pag-IBIG MP2','pagibig_housing':'Pag-IBIG Housing','provident_fund':'Provident Fund','provident_fund_loan':'Provident Fund Loan','provident_edu_loan':'Provident Edu Loan','provident_term_loan':'Provident Term Loan','bacgem_edu_loan':'BACGEM Edu Loan','bacgem_grocery':'BACGEM Grocery','bacgem_others':'BACGEM Others','bacgem_hcp':'BACGEM HCP','bacgem_loan':'BACGEM Loan','nocgem_edu_loan':'NOCGEM Edu Loan','nocgem_emergency':'NOCGEM Emergency','nocgem_grocery':'NOCGEM Grocery','nocgem_hospital':'NOCGEM Hospital','nocgem_others':'NOCGEM Others','nocgem_plp':'NOCGEM PLP','nocgem_regular_loan':'NOCGEM Regular Loan','other_cash_advance':'Cash Advance','other_insurance':'Insurance','other_salary_loan':'Salary Loan','other_others':'Other Deductions'};
    var bcgeuVal=getBcgeuForPeriod();
    var rows='';var grandTotal=0;
    rows+='<tr><td class="dl">BCGEU</td><td class="dr">'+fmtP(bcgeuVal)+'</td></tr>';grandTotal+=bcgeuVal;
    Object.entries(blocks).forEach(function([section,fields]){fields.forEach(function(fname){var inp=document.querySelector('input[name="emp_'+fname+'['+empId+']"]');var val=inp?(parseFloat(inp.value)||0):0;if(val>0){rows+='<tr><td class="dl">'+(labels[fname]||fname)+'</td><td class="dr">'+fmtP(val)+'</td></tr>';grandTotal+=val;}});});
    function fmtP(n){return parseFloat(n).toLocaleString('en-PH',{minimumFractionDigits:2,maximumFractionDigits:2});}
    var html='<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Loan Summary — '+fullName+'</title><style>body{font-family:Arial,sans-serif;font-size:12px;margin:0;padding:20px;color:#111;}.header{text-align:center;margin-bottom:18px;border-bottom:2px solid #1a3a5c;padding-bottom:10px;}.header h2{margin:0 0 2px;font-size:15px;color:#1a3a5c;}.header p{margin:2px 0;font-size:11px;color:#555;}.info-grid{display:grid;grid-template-columns:1fr 1fr;gap:4px 24px;margin-bottom:16px;padding:10px 14px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;}.info-grid .lbl{font-size:10px;color:#6b7280;text-transform:uppercase;letter-spacing:.04em;}.info-grid .val{font-size:12px;font-weight:700;color:#111;margin-bottom:6px;}table{width:100%;border-collapse:collapse;margin-top:4px;}thead tr{background:#1a3a5c;color:#fff;}thead th{padding:7px 10px;text-align:left;font-size:11px;letter-spacing:.04em;}thead th.dr{text-align:right;}tbody tr:nth-child(even){background:#f8fafc;}tbody td{padding:6px 10px;border-bottom:1px solid #e5e7eb;font-size:11.5px;}td.dl{color:#374151;}td.dr{text-align:right;font-weight:600;color:#1a3a5c;}.total-row td{border-top:2px solid #1a3a5c;font-weight:800;font-size:13px;padding:8px 10px;}.footer{margin-top:18px;font-size:10px;color:#9ca3af;text-align:center;border-top:1px solid #e5e7eb;padding-top:8px;}@media print{body{padding:10px;}}</style></head><body><div class="header"><h2>Loan &amp; Deduction Summary</h2><p>Payroll Period: <strong>'+period+'</strong></p></div><div class="info-grid"><div><div class="lbl">Employee Name</div><div class="val">'+fullName+'</div></div><div><div class="lbl">Employee ID</div><div class="val">'+employeeId+'</div></div><div><div class="lbl">Position</div><div class="val">'+position+'</div></div><div><div class="lbl">Salary Grade / Step</div><div class="val">SG-'+sg+' &nbsp;·&nbsp; Step '+step+'</div></div><div><div class="lbl">Basic Salary</div><div class="val">&#8369; '+fmtP(basicSalary)+'</div></div></div><table><thead><tr><th>Deduction Item</th><th class="dr">Amount</th></tr></thead><tbody>'+rows+'</tbody><tfoot><tr class="total-row"><td>TOTAL DEDUCTIONS</td><td class="dr">&#8369; '+fmtP(grandTotal)+'</td></tr></tfoot></table><div class="footer">Generated on '+new Date().toLocaleString('en-PH')+' &nbsp;·&nbsp; Payroll System</div></body></html>';
    var win=window.open('','_blank','width=600,height=700');win.document.write(html);win.document.close();win.focus();setTimeout(function(){win.print();},400);
}
</script>
